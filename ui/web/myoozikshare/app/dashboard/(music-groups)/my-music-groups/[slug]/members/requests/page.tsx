...
import { MembershipRequestsTableBodySkeleton } from "@/app/ui/dashboard/music-groups/skeletons";
...
import MembershipRequestsTableBody from "@/app/ui/dashboard/music-groups/members/membership-requests-table-body";
    ...
    return (
      <>
        ...
        <div className="mt-10 mb-2">
          <Link href={`/dashboard/my-music-groups/${musicGroup.id}/members`}
        ...
        <SearchForm />
        <div className="w-full my-2 flex justify-center items-center text-2xl font-bold">Membership Requests
        </div>
        <Table className="relative table-auto overflow-x-scroll">
          <TableHeader>
            ...
            <TableRow className="bg-black hover:bg-black text-white">
              ...
              <TableHead className="text-white dark:text-amber-500">Status</TableHead>
              <TableHead className="text-right text-white dark:text-amber-500">Actions</TableHead>
            </TableRow>
          </TableHeader>
          <Suspense key={query + currentPage} fallback={<MembershipRequestsTableBodySkeleton />}>
          <MembershipRequestsTableBody slug={slug} musicGroup={musicGroup} query={query} currentPage={currentPage} />
          </Suspense>
        </Table>
      </>
  );
